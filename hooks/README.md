...
## Nouveaux Hooks (Architecture Refactorée)

### useLocation

Gère la localisation de l'utilisateur avec cache et refresh.
...
// refreshLocation(): Force le refresh de la localisation
```

### useWeather

Récupère les données météo avec cache de 30 minutes.
...
Gère les horaires de prière avec fallback sur calcul local.

**Usage :**
```typescript
import { usePrayerTimes } from '@/hooks';

const { prayerTimes, isLoading, error, refreshPrayerTimes, getNextPrayer } = usePrayerTimes(
  location?.latitude,
  location?.longitude
);

const nextPrayer = getNextPrayer();
// { prayer: PrayerTimeEntry, minutesUntil: number }
```

### useHijriDate

Calcul mémorisé de la date Hijri avec cache.

**Usage :**
```typescript
import { useHijriDate } from '@/hooks';

const hijriDate = useHijriDate(new Date());
// { day, month, year, formattedFr, formattedAr, isRamadan, ramadanDay }
```

### useDailyReset

Reset automatique quotidien des habitudes, dhikr et progression Coran.

**Usage :**
```typescript
import { useDailyReset } from '@/hooks';

// Dans le root layout
function RootLayout() {
  useDailyReset(); // Vérifie et reset automatiquement à minuit
