...
async function getTranslationCache(): Promise<Record<string, CachedTranslation>> {
  try {
    ...
    console.log('[Translation] Cache read error:', err);
    return {};
  }
}
...
async function saveToCache(french: string, arabic: string, transliteration: string): Promise<void> {
  try {
    ...
    console.log('[Translation] Cache save error:', err);
  }
}
...
async function translateOnline(text: string): Promise<TranslationResult> {
  try {
    console.log('[Translation] Translating online:', text);
    ...
    console.log('[Translation] Success:', { arabic, transliteration });
    ...
    console.log('[Translation] Online translation error:', err);
    throw err;
  }
}
...
async function addToPendingQueue(french: string): Promise<void> {
  try {
    ...
    if (!queue.find(p => p.french.toLowerCase() === french.toLowerCase())) {
      ...
      console.log('[Translation] Added to pending queue:', french);
    }
  } catch (err) {
    console.log('[Translation] Pending queue error:', err);
  }
}
...
export async function translateToArabic(text: string): Promise<TranslationResult> {
  ...
  if (commonTerm) {
    console.log('[Translation] Found in common terms:', text);
    return commonTerm;
  }
  ...
  if (cached) {
    console.log('[Translation] Found in cache:', text);
    return {
  ...
  try {
    ...
    console.log('[Translation] Offline, adding to queue:', text);
...
export async function syncPendingTranslations(): Promise<number> {
  try {
    ...
    console.log('[Translation] Syncing', queue.length, 'pending translations');
    ...
    console.log('[Translation] Synced', synced, 'translations,', remaining.length, 'remaining');
    return synced;
  } catch (err) {
